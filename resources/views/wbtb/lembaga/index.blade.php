...
@section('page-title', 'Data Lembaga Kebudayaan')
    ...
    <div class="p-4 sm:p-6">
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
            <div class="flex flex-col space-y-2 w-full sm:w-auto">
                @if (Auth::user()->role != 'user')
                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                        <a href="{{ route('wbtb.lembaga.create') }}" 
            ...
            <div class="flex flex-col w-full sm:w-auto space-y-4">
                <!-- Form Filter -->
                <form action="{{ route('wbtb.lembaga.index') }}" method="GET" class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full">
                    <select name="kategori" class="rounded-md border-gray-300 shadow-sm text-sm sm:text-base">
                        <option value="">Semua Kategori</option>
                        <option value="Sanggar" {{ request('kategori') == 'Sanggar' ? 'selected' : '' }}>Sanggar</option>
                        <option value="Komunitas" {{ request('kategori') == 'Komunitas' ? 'selected' : '' }}>Komunitas</option>
                        <option value="Organisasi" {{ request('kategori') == 'Organisasi' ? 'selected' : '' }}>Organisasi</option>
                        <option value="Yayasan" {{ request('kategori') == 'Yayasan' ? 'selected' : '' }}>Yayasan</option>
                        <option value="Perkumpulan" {{ request('kategori') == 'Perkumpulan' ? 'selected' : '' }}>Perkumpulan</option>
                        <option value="Lainnya" {{ request('kategori') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                    
                    <select name="tingkat" class="rounded-md border-gray-300 shadow-sm text-sm sm:text-base">
                ...
                <!-- Form Pencarian -->
                <form action="{{ route('wbtb.lembaga.index') }}" method="GET" class="w-full sm:w-auto">
                    <div class="relative">
        ...
        <div class="overflow-x-auto">
            ...
            <div class="hidden sm:block">
                <table class="min-w-full bg-white border border-gray-200">
                    ...
                    <tbody>
                        @forelse ($lembagaItems as $lembaga)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                                    <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">
                                        {{ $lembaga->kategori_lembaga }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                                    {{ $lembaga->tingkat_lembaga ?? '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                                    {{ $lembaga->nama_pimpinan }}
                                    @if($lembaga->jabatan_pimpinan)
                                        <span class="text-gray-500 text-xs">({{ $lembaga->jabatan_pimpinan }})</span>
                                    @endif
                                </td>
                                @if (Auth::user()->role != 'user')
                                    <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-sm">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('wbtb.lembaga.show', $lembaga->id) }}" class="text-blue-600 hover:text-blue-900">
                                        ...
                                        @if (Auth::user()->role != 'user')
                                            @if ((Auth::user()->role == 'admin' && !$lembaga->is_verified) || Auth::user()->role == 'superadmin')
                                                <a href="{{ route('wbtb.lembaga.edit', $lembaga->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @if (Auth::user()->role == 'superadmin')
                                                @if (!$lembaga->is_verified)
                                                    <form action="{{ route('wbtb.lembaga.verify', $lembaga->id) }}" method="POST" class="inline">
                                                        @csrf
                                                ...
                                                @endif
                                                <form action="{{ route('wbtb.lembaga.destroy', $lembaga->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ Auth::user()->role != 'user' ? 6 : 5 }}" class="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-center">
                                    Tidak ada data lembaga kebudayaan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                @forelse ($lembagaItems as $lembaga)
                    <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                        <div class="grid grid-cols-1 gap-2">
                            <div>
                                <span class="text-xs text-gray-600 uppercase">Nama Lembaga</span>
                                <p class="text-sm font-medium">{{ $lembaga->nama_lembaga }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <span class="text-xs text-gray-600 uppercase">Tingkat</span>
                                <p class="text-sm">{{ $lembaga->tingkat_lembaga ?? '-' }}</p>
                            </div>
                            <div>
                                <span class="text-xs text-gray-600 uppercase">Pimpinan</span>
                                <p class="text-sm">
                                    {{ $lembaga->nama_pimpinan }}
                                    @if($lembaga->jabatan_pimpinan)
                                        <span class="text-gray-500 text-xs">({{ $lembaga->jabatan_pimpinan }})</span>
        ...
        <div class="mt-4">
            {{ $lembagaItems->appends(request()->query())->links() }}
        </div>
    </div>
</div>
@endsection